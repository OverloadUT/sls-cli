 ...
 */
export async function traverseDirectory(dirPath, options, currentDepth = 0, visitedInodes = new Set()) {
    ...
    if (stats.isFile()) {
        ...
        if (!options.noDescriptions && dirPath.endsWith('.md')) {
            ...
            if (frontMatter.purpose)
                entry.purpose = frontMatter.purpose;
            if (frontMatter.tags)
    ...
    try {
        ...
        for (const item of filteredItems) {
            ...
            try {
                ...
                const childEntry = await traverseDirectory(itemPath, options, currentDepth + 1, new Set(visitedInodes));
